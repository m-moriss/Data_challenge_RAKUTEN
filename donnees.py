...
def load_dataset():
    ...
    X_train['imageid'] = X_train['imageid'].str.replace(',', '').astype(int)
    X_train['productid'] = X_train['productid'].str.replace(',', '').astype(int)
    ...
    X["image"] = "image_" + X.imageid.astype(str) + "_product_" + X.productid.astype(str) + ".webp"
